...
toks_tweet <- tokens(corp, remove_punct = TRUE) %>% 
    tokens_tolower() %>%
    tokens_remove(pattern = stopwords("en")) %>%
    filter(str_extract_all(c("h@i", "hi @hello @me", "@twitter"), "(?<=^|\\s)@[^\\s]+"))
    tokens_select(min_nchar=8)

## pre-process: make document-frequency matrix (words)
dfm_tweet <- dfm(toks_tweet)
...
tweet_df <- read_csv("10-Campaigns/trumptweets_2016-2020.csv") %>% 
    mutate(text = map(text, ~str_extract(.x, "@")))

%>% 
    filter(yes == TRUE)

corp <- corpus(tweet_df)


toks_tweet <- tokens(tweet_df, remove_punct = TRUE) %>% 
    tokens_tolower() %>%
    tokens_remove(pattern = stopwords("en"))
